...
#elif CONFIG_OPPO_FINGERPRINT_PLATFORM == 6768 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6771
fp_module_config_t fp_module_config_list[] = {
...
static int fp_register_proc_fs(struct fp_data *fp_data)
{
    ...
    fp_id_retry = 0;
    int igetpcbversionfp = 0;

#if CONFIG_OPPO_FINGERPRINT_PLATFORM == 6785 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6768
	...
	igetpcbversionfp = get_PCB_Version();
	dev_err(fp_data->dev, "[zhaojunhai]fp_register_proc_fs get_PCB_Version: %d\n", igetpcbversionfp);
	if(igetpcbversionfp < HW_VERSION__EVT1){
		if(fp_id_pull_up_value == 1 && fp_id_pull_down_value == 0){
			fp_data->fp_id0 = 2;
		}else if(fp_id_pull_up_value == 0 && fp_id_pull_down_value == 0){
			fp_data->fp_id0 = 0;
		...
		dev_err(fp_data->dev, "[zhaojunhai]fp_register_proc_fs: < EVT1\n");
	}else if(igetpcbversionfp >= HW_VERSION__EVT1){
		if(fp_id_pull_up_value == 1 && fp_id_pull_down_value == 0){
			fp_data->fp_id0 = 1;
		}else if(fp_id_pull_up_value == 0 && fp_id_pull_down_value == 0){
			fp_data->fp_id0 = 0;
