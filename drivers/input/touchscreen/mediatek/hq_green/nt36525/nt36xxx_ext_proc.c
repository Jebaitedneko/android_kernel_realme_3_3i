...
static ssize_t oppo_gesture_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
	...
	if (len > count) {
		NVT_ERR("failed to copy_to_user, len is too long\n");
		return 0;
	}

	if (copy_to_user(buf, ptr, len)) {
		NVT_ERR("failed to copy_to_user\n");
		return 0;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	NVT_ERR("game switch enable is %d\n", value);
	tmp = !!tmp;
	g_switch = tmp;
		...
		size_t count, loff_t *ppos)
{
	...
	NVT_LOG("enter oppo_tp_limit_write\n");
	if(copy_from_user(cmd, buf, count)) {
		NVT_ERR("input value error\n");
		return -EINVAL;
	}
